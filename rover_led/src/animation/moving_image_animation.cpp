...
#include "rover_led/animation/moving_image_animation.hpp"
    ...
    const float controller_frequency)
{
    ...
    try {
        image_center_offset_ = rover_utils::getYAMLKeyValue<std::int16_t>(animation_description, "center_offset");
    } catch (const std::runtime_error & /*e*/) {
        RCLCPP_DEBUG(rclcpp::get_logger("MovingImageAnimation"),
            "Missing center_offset in animation description, using default value 0");
        image_center_offset_ = 0;
    }

    try {
        image_object_width_ = rover_utils::getYAMLKeyValue<std::int16_t>(animation_description, "object_width");
    } catch (const std::runtime_error & /*e*/) {
        RCLCPP_DEBUG(rclcpp::get_logger("MovingImageAnimation"),
    ...
    try {
        float image_start_offset_time = std::clamp(rover_utils::getYAMLKeyValue<float>(animation_description, "start_offset"), -20.0f, 20.0f);
    ...
    try {
        ...
        rover_utils::getYAMLKeyValue<float>(animation_description, "splash_duration"), 0.0f, 20.0f);  // in seconds
    ...
    try {
        image_mirrored_ = rover_utils::getYAMLKeyValue<bool>(
    ...
    try {
        position_mirrored_ = rover_utils::getYAMLKeyValue<bool>(
    ...
    try {
        default_image_position_ = std::clamp(rover_utils::getYAMLKeyValue<float>(animation_description, "default_image_position"), 0.0f, 1.0f);
        default_image_position_set_ = true;
    } catch (const std::runtime_error & /*e*/) {
        RCLCPP_DEBUG(rclcpp::get_logger("MovingImageAnimation"),
            "Missing default_image_position in animation description, will throw if no param is provided");
    }
...
void MovingImageAnimation::setParam(const std::string & param)
{
    if (default_image_position_set_ && param.empty()) {
        ...
        RCLCPP_DEBUG(rclcpp::get_logger("MovingImageAnimation"), "Setting image position to default: %f", default_image_position_);
        return;
    }
