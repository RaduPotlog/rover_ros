...
void RoverModbus::writeMultipleRegisters(const std::vector<uint8_t> &register_values, const uint16_t start_address)
{
    if (write_to_modbus_mtx_.try_lock()) {
        std::lock_guard<std::mutex> lck(write_to_modbus_mtx_, std::adopt_lock);
    
        std::vector<MB::ModbusCell> value = {
            MB::ModbusCell(static_cast<uint16_t>(0xFFFF))
        };
        ...
        try {
            ...
            std::cerr << "Failed to send request to the led controller" << std::endl;
            throw;
        }
    } else {
        std::cerr << "Failed to acquire lock for writing to modbus. Another write operation is in progress." << std::endl;
        return;
    }
}
