...
4. After a fixed 10 s delay (giving the hardware interface time to come up), starts:
   ...
   - `rover_localization` (with `use_ekf:=True`)
   - `rover_crfs_teleop`
   - `rover_twist_mux`
...
- `rover_ublox_launch` (the `ublox_gps` GNSS driver) is defined in `rover_bringup.launch.py` but
  not added to the launch actions, so no GNSS driver is started.
