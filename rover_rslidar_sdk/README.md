...
> **Rover fork.** This is a vendored copy of RoboSense's
> [rslidar_sdk](https://github.com/RoboSense-LiDAR/rslidar_sdk), renamed to
> `rover_rslidar_sdk` so the ROS package matches the folder it lives in. The
> executable is `rover_rslidar_sdk_node` and it is built and tested on
> **ROS 2 Lyrical**; the upstream instructions below still apply otherwise.

 [中文介绍](README_CN.md)

## 1 Introduction

**rover_rslidar_sdk** is the Software Development Kit of the RoboSense Lidar based on Ubuntu. It contains:
...
Download the rover_rslidar_sdk as below. Since it contains the submodule rs_driver, please also use `git submodule` to download the submodule properly.
...
To run rover_rslidar_sdk in the ROS environment, please install below libraries.
...
To use rover_rslidar_sdk in the ROS2 environment, please install below libraries.

+ Ubuntu 16.04 - Not supported
+ Ubuntu 18.04 - ROS2 Eloquent desktop
+ Ubuntu 20.04 - ROS2 Galactic desktop
+ Ubuntu 22.04 - ROS2 Humble desktop
+ ROS2 Lyrical (what this fork is built against)
...
(1) Create a new workspace folder, and create a *src* folder in it. Then put the rover_rslidar_sdk project into the *src* folder.
...
roslaunch rover_rslidar_sdk start.launch
```

### 4.2 Compile with ROS2 colcon

(1) Create a new workspace folder, and create a *src* folder in it. Then put the rover_rslidar_sdk project in the *src* folder.
...
ros2 launch rover_rslidar_sdk start.py
```
...
To change behaviors of rover_rslidar_sdk, change its parameters. please read the following links for detail information.
...
Below are some quick guides to use rover_rslidar_sdk.
