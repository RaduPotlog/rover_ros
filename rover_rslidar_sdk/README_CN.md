...
> **Rover 定制版。** 本工程是 RoboSense
> [rslidar_sdk](https://github.com/RoboSense-LiDAR/rslidar_sdk) 的内置副本，
> 已重命名为 `rover_rslidar_sdk`，使 ROS 包名与所在目录一致。可执行文件为
> `rover_rslidar_sdk_node`，基于 **ROS 2 Lyrical** 构建与验证；其余部分仍与
> 上游说明一致。
 ...
 **rover_rslidar_sdk** 是速腾聚创在Ubuntu环境下的雷达驱动软件包。它包括：
...
(1) 新建一个文件夹作为工作空间，然后再新建一个名为*src*的文件夹, 将rover_rslidar_sdk工程放入*src*文件夹内。
...
rover_rslidar_sdk的功能通过配置参数文件来实现，请仔细阅读。
