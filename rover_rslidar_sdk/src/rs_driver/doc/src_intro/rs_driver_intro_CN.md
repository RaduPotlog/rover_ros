...
RSEMX是EM平台雷达。这里说明RSEMX的Decoder。

+ DecoderRSEMX的常量配置由成员函数getConstParam()生成。这个配置定义为静态本地变量。
+ 成员 `split_strategy_`是SplitStrategyBy类的实例，保存分帧策略。
...
|     常量参数     |                        值                        | 说明                                   |
| :---------------: | :----------------------------------------------: | :------------------------------------- |
|     MSOP_LEN     |                       812                       | MSOP Packet字节数                      |
|     DIFOP_LEN     |      654（多种长度，不使用此变量检查包长）      | DIFOP Packet字节数                     |
|     MSOP_ID[]     |             {0x55, 0xAA, 0x5A, 0xA5}             | MSOP Packet标志字节，长度为4           |
|    MSOP_ID_LEN    |                        4                        | MSOP_LEN[]中标志字节的长度             |
...
|  BLOCKS_PER_PKT  |                       192                       | 每Packet中192个Block                   |
| CHANNEL_PER_BLOCK |                        1                        | RSEMX有192个通道，每个Block对应1个通道 |
|   DISTANCE_MIN   |                       0.2                       | 测距最小值，单位米                     |
|   DISTANCE_MAX   |                      300.0                      | 测距最大值，单位米                     |
|   DISTANCE_RES   |                      0.005                      | Packet中distance的解析度，单位米       |
|  TEMPERATURE_RES  |                        80                        | 雷达温度的初始值，EMX温度计算未使用    |

需要注意，对于EMX，每个MSOP内连续保存192个channel的数据，单回波模式下每一包192个channel构成1列。为沿用之前结构，每包写作192个block，每个block 1个通道。事实上，改写为每包1个block，每个192个通道也是同样解析。

##### 4.8.6.2 DecoderRSEMX::decodeDifopPkt()

decodeDifopPkt() 解析DIFOP2 Packet，有多种长度可能。

+ 在DIFOP2中，解析得到pitch角度pitch_angle，DIFOP2有多种。

##### 4.8.6.3 DecoderRSEMX::decodeMsopPkt()
...
+ 遍历Packet内所有的Block。
  ...
  + 从Block相对于Packet的偏移，得到Block的时间戳。Block内的所有Channel的时间戳都是这个时间戳，当然 RSEMX每Block仅1个channel，为通用写法。
  + 单回波每1包、双回波每2包构成1列192通道，计算得到通道号real_chan。根据real_chan得到pitch。
  + 用每包的16位yaw_angle以及系数计算得到这一列点的yaw角度。
+ 遍历Block内所有的Channel。
...
与MSOP/DIFOP Packet的录制与回放相关的逻辑，散布在rs_driver的各个模块中，所以这里单独分一个章节说明。
