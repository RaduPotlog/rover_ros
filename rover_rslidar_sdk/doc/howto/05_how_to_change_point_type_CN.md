...
`POINT_TYPE`为`XYZI`时，rover_rslidar_sdk使用RoboSense自定义的点类型```PointXYZI```. 
...
rover_rslidar_sdk将基于`PointXYZI`的点云，转换为ROS的`PointCloud2`消息，再发布出去。
...
`POINT_TYPE`为`XYZIRT`时，rover_rslidar_sdk使用RoboSense自定义的点类型```PointXYZRT```。
...
rover_rslidar_sdk将基于`PointXYZIRT`的点云，转换为ROS的PointCloud2消息，再发布出去。
...
`POINT_TYPE`为`XYZIF`时，rover_rslidar_sdk使用RoboSense自定义的点类型```PointXYZIF```。
...
rover_rslidar_sdk将基于`PointXYZIF`的点云，转换为ROS的PointCloud2消息，再发布出去。
...
`POINT_TYPE`为`XYZIRTF`时，rover_rslidar_sdk使用RoboSense自定义的点类型```PointXYZIRTF```。
...
rover_rslidar_sdk将基于`PointXYZIRTF`的点云，转换为ROS的PointCloud2消息，再发布出去。
