...
本文描述在这些场景下如何配置rover_rslidar_sdk。
...
+ 雷达发送Packet到 `255.255.255.255` : `6699`, rover_rslidar_sdk绑定到主机的端口 `6699`.
...
为了减少网络负载，建议雷达使用单播模式。
+ 雷达发送Packet到 `192.168.1.102` : `6699`, rover_rslidar_sdk绑定端口 `6699`。
...
+ rover_rslidar_sdk绑定到端口 `6699`。同时它将IP地址为`192.168.1.102`的本地网络接口加入组播组`224.1.1.1`。
...
+ 第一个雷达发送Packet到 `192.168.1.102`:`6699`, 给rover_rslidar_sdk配置的第一个driver节点绑定到`6699`。
+ 第二个雷达发送Packet到 `192.168.1.102`:`5599`, 给rover_rslidar_sdk配置的第二个driver节点绑定到`5599`。
...
+ 第一个雷达发送Packet到 `192.168.1.102`:`6699`, 给rover_rslidar_sdk配置的第一个driver节点绑定到`192.168.1.102:6699`。
+ 第二个雷达发送Packet到 `192.168.1.103`:`6699`, 给rover_rslidar_sdk配置的第二个driver节点绑定到`192.168.1.103:6699`。
...
现在，rover_rslidar_sdk可以将`eno1.80`当做一个一般的网卡来处理，从这里接收不带VLAN层的Packet.
...
这两个层是UDP数据的一部分，所以rover_rslidar_sdk可以自己剥除它们。只需要指出这两个层的长度就可以了。
