^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Changelog for package rover_io_context
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

2.0.0 (2026-09-17)
------------------
* Forked from ros-drivers/transport_drivers v1.2.0 (package ``io_context``).
* Renamed from ``io_context`` and relayouted into domain / application / infrastructure.
* Gained the shared ``ByteStreamPort`` / ``BytePublisherPort`` ports and the inbound / outbound byte bridges.
* Dropped the message converters (moved to the drivers), the dead and broken ``std_msgs.cpp``, the ``PUBLIC "-O0"`` compile option, and the rclcpp dependency.
