^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Changelog for package rover_udp_driver
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

2.0.0 (2026-09-17)
------------------
* Forked from ros-drivers/transport_drivers v1.2.0 (package ``udp_driver``).
* Renamed from ``udp_driver`` and relayouted into domain / infrastructure.
* Fixed a use-after-free in the async send path, a receiver that stopped on a zero-length datagram, a receive buffer resized under the read, and ``-1`` returned from ``std::size_t`` functions.
* Dropped ``UdpDriver``, ``visibility_control.hpp`` and the rclcpp_components registration.
