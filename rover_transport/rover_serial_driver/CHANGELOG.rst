^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Changelog for package rover_serial_driver
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

2.0.0 (2026-09-17)
------------------
* Forked from ros-drivers/transport_drivers v1.2.0 (package ``serial_driver``).
* Renamed from ``serial_driver`` and relayouted into domain / infrastructure.
* Fixed a use-after-free in the async send path and a ``-Wshadow`` violation in the receive re-arm.
* Parameter parsing and validation moved into the domain layer and unit-tested.
* Dropped ``SerialDriver``, ``visibility_control.hpp`` and the rclcpp_components registration.
