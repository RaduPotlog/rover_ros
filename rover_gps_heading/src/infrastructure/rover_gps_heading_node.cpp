...
, diagnostic_updater_(std::make_shared<diagnostic_updater::Updater>(this))
{
    ...
    // Reject inconsistent combinations at startup, not at the first fix.
    domain::HeadingAlignmentEstimator::validate(alignment_config_);
}

void RoverGpsHeadingNode::init()
{
    diagnostic_updater_->setHardwareID("RoverGpsHeading");
    ...
    reset_alignment_service_ = create_service<std_srvs::srv::Trigger>(
        ...
        std::bind(&RoverGpsHeadingNode::resetAlignmentCallback, this, _1, _2));

    tick_timer_ = create_wall_timer(1s, std::bind(&RoverGpsHeadingNode::tickCallback, this));

    // Publish the unaligned status immediately instead of after the first timer period.
    tickCallback();
}

void RoverGpsHeadingNode::fixCallback(const infrastructure::NavSatFixMsg::SharedPtr msg)
{
...
void RoverGpsHeadingNode::odometryCallback(const infrastructure::OdometryMsg::SharedPtr msg)
{
    align_heading_->onOdometry(infrastructure::toOdometrySample(*msg, nowSeconds()));
}

void RoverGpsHeadingNode::tickCallback()
{
    align_heading_->onTick();
}

void RoverGpsHeadingNode::resetAlignmentCallback(
...
double RoverGpsHeadingNode::nowSeconds() const
{
