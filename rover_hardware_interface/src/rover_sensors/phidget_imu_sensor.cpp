...
CallbackReturn PhidgetImuSensor::on_configure(const rclcpp_lifecycle::State &)
{
    ...
    RCLCPP_DEBUG_STREAM(logger_, "\tcallback_delta_epsilon_ms " << params_.callback_delta_epsilon_ms << "ms");

    try {
        readMadgwickFilterParams();
        configureMadgwickFilter();
    } catch (const std::exception & e) {
        RCLCPP_ERROR_STREAM(logger_, "An exception occurred while reading the Madgwick Filter parameters: " << e.what());
        return CallbackReturn::ERROR;
    }
...
CallbackReturn PhidgetImuSensor::on_activate(const rclcpp_lifecycle::State &)
{
    ...
    RCLCPP_DEBUG_STREAM(logger_, "Connecting to Phidgets Spatial - serial: " << params_.serial << ", hub port: " << params_.hub_port);
...
std::vector<StateInterface> PhidgetImuSensor::export_state_interfaces()
{
    ...
    for (size_t i = 0; i < kImuInterfacesSize; i++) {
        state_interfaces.emplace_back(info_.sensors.at(0).name, info_.sensors.at(0).state_interfaces.at(i).name, &imu_sensor_state_.at(i));
    }
...
void PhidgetImuSensor::checkInterfaces() const
{
    ...
    if (!std::equal(names_start_iter, names_end_iter, states_iter, compare_name_with_interface_info)) {
...
{
    if (areParamsDefined({"cc_mag_field", "cc_offset0", "cc_offset1", "cc_offset2", "cc_gain0", "cc_gain1",
        ...
        RCLCPP_INFO(logger_, "Compass correction parameters not found. Skipping compass configuration.");
    }
}
    ...
    const rclcpp::Time & timestamp)
{
    ...
    if (!StatelessOrientation::computeOrientation(world_frame_, lin_acc, mag_compensated, init_q)) {
        throw std::runtime_error("Couldn't determine gravity direction. The IMU sensor seems to be in free fall.");
    }
    ...
    const double timestamp)
{
    ...
    if (!algorithm_initialized_ || params_.stateless) {
        ...
        try {
            ...
            RCLCPP_ERROR_STREAM(logger_, "An exception occurred while initializing the Madgwick algorithm: " << e.what());
        }
    }
...
PLUGINLIB_EXPORT_CLASS(
  rover_hardware_interface::PhidgetImuSensor, hardware_interface::SensorInterface)