...
// The sweep cost must stay at four round-trips (the contacts, plus one per one-byte coil block)
...
TEST_F(HeartbeatTimingTest, OneIoSweepCostsFourReadTransactions)
{
    ...
    EXPECT_EQ(modbus->readTransactionCount(), 4u);
}
