...
SafetyBtTickAfterTimeout::SafetyBtTickAfterTimeout(const std::string & name, const BT::NodeConfig & conf)
: BT::DecoratorNode(name, conf)
{
    this->last_success_time_ = std::chrono::steady_clock::now();
}

BT::NodeStatus SafetyBtTickAfterTimeout::tick()
{
    float timeout;
...
{
    factory.registerNodeType<rover_safety::SafetyBtTickAfterTimeout>("SafetyBtTickAfterTimeout");
}